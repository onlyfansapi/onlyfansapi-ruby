...
onlyfansapi.whoami.retrieve(request_options: {max_retries: 5})
```
...
# Or, configure per-request:
onlyfansapi.whoami.retrieve(request_options: {timeout: 5})
```
...
puts(whoami[:my_undocumented_property])
```
...
onlyfansapi.whoami.retrieve(**params)
```
